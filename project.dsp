...
queBanda(hz) = ba.if((hz>=130.5) & (hz<=261),1,0) + ba.if((hz>=261) & (hz<=523),2,0) + ba.if((hz>=523.25) & (hz<=1046.3),3,0) + ba.if((herzios>=1046.3) & (herzios<=2093),4,0) + ba.if((herzios>=2093) & (herzios<=4186),5,0);
...
media = _*256 + _*510 + _*1020 + _*2040 + _*4080: _ / 5: inRange: _ <: attach(_, hbargraph("Los hz que se estan generando ",0,5000));


// main functions
recursividad = _:queBanda:hbargraph("La banda en la que se estan generando ",0,7);
//main
process = os.osc(herzios): (spectral_level_demo: printar: media) ~ (recursividad): os.osc: _; 
